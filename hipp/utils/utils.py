...
def optimize_geotif(
    geotif_file_name, output_file_name=None, verbose=False, print_call=False
):

    if output_file_name is None:
        file_path, file_name, file_extension = hipp.io.split_file(geotif_file_name)
        output_file_name = os.path.join(
            file_path, file_name + "_optimized" + file_extension
        )
...
def optimize_geotifs(input_directory, keep=False, verbose=False):
...
def enhance_geotif_resolution(
    ...
    if output_file_name is None:
        file_path, file_name, file_extension = hipp.io.split_file(geotif_file_name)
        output_file_name = os.path.join(
            file_path, file_name + "_high_res" + file_extension
        )
