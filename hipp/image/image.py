...
def affine_transform_image(image_array, coordinates, coordinates_true, order=3):
    """
    ...
    image_array_transformed = tf.warp(
        image_array, A, output_shape=output_dim, order=order
    )
...
def clahe_equalize_image(img_gray, clipLimit=2.0, tileGridSize=(8, 8)):
...
def crop_about_point(image_array, point_yx, image_square_dim=11250):

    distance_from_point = int(
        round(image_square_dim / 2)
