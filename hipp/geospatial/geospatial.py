...
def EE_image_corner_coords_to_polygon_gdf(df, epsg_code="4326"):
    ...
    for index, row in df_tmp.iterrows():
        ...
        if (
            lons[0] == lons[1]
        ):  # handles EarthExplorer instances where corner coordinates are identical
            polygon = (
                gpd.GeoDataFrame(
                    geometry=[
                        polygon,
                ...
                .to_crs("epsg:" + epsg_code)["geometry"]
                .buffer(0.018)
                .to_crs("epsg:" + "4326")
                .iloc[0]
            )
    ...
    gdf = gpd.GeoDataFrame(df_tmp, geometry=polygons, crs="epsg:" + "4326").to_crs(
        "epsg:" + epsg_code
    )
    return gdf
