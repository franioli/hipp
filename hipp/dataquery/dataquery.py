...
def download_image(output_directory, payload, default_img_ext=".tif"):
...
def thread_downloads(output_directory, urls, file_names, max_workers=5):
    with tqdm(total=len(urls)) as pbar:
        ...
        future_to_url = {
            pool.submit(download_image, output_directory, x): x
            for x in zip(urls, file_names)
        }
        results = []
...
def EE_download_images_to_disk(
    ...
    if filenames_med or filenames_hi:
        ...
        calibration_reports_directory = os.path.join(
            output_directory, calibration_reports_directory_suffix
        )
        ...
        if not pathlib.Path(images_directory).is_dir() or overwrite:
            ...
            thread_downloads(output_directory, urls, filenames, max_workers=max_workers)
            ...
            with tqdm(total=len(urls)) as pbar:
                pool = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)
                reorged_futures = {
                    pool.submit(fix_grid_org, x): x for x in original_raw_tif_files
                }
...
def EE_convert_api_responses_to_dataframe(scenes):
    ...
    for scene in scenes:
        ...
        one_scene_df = one_scene_df[
            one_scene_df.fieldName.isin(api_to_hsfm_field_name_dict.keys())
        ]
        one_scene_df = (
            one_scene_df[["fieldName", "value"]]
            .set_index("fieldName")
            .transpose()
...
def EE_login(username, token, m2mhost="https://m2m.cr.usgs.gov/api/api/json/stable/"):
...
def EE_pre_select_images(
    ...
    scenes = EE_sendRequest(
        serviceUrl + "scene-search", datasetSearchParameters, apiKey
    )
...
def EE_stageForDownload(
    ...
    if downloads:
        ...
        if (
            ...
            payload = {"label": label}
            moreDownloadUrls = EE_sendRequest(
...
def NAGAP_pre_select_images(
    ...
    if (
        len(list(set(df["Year"].values))) == 1
        and len(list(set(df["Month"].values))) > 1
    ):
